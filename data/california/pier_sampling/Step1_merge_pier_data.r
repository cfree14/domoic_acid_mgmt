...
# Read master data
data1_orig <- read.csv(file.path(indir, "HABs-pre20190601_71e8_1801_a678.csv"), as.is=T, na.strings = c("NaN", "NA"))
...
data1 <- data1_orig %>%
  ...
  rename("lat_dd"="latitude",
         ...
         "pda_ng_ml"="pDA", # particulate domoic acid
         "tda_ng_ml"="tDA", # total domoic acid
         "dda_ng_ml"="dDA", # dissolved domoic acid
...
data2_orig <- purrr::map_df(files2merge, function(x){
  ...
  # Read data
  fdata_orig <- read.csv(file.path(indir, x), na.strings=c("NA", "NaN"))

...
data_key <- purrr::map_df(loc_key$location, function(x){
  s <- date_key %>%
    mutate(location=x,
           pn_group="Seriata") %>%
    select(location, pn_group, everything())
  d <- date_key %>%
    mutate(location=x,
           pn_group="Delicatissima") %>%
    select(location, pn_group, everything())
  df <- bind_rows(s, d)
})
...
data <- data_orig %>%
  # After 2014
  filter(date >= ymd("2014-01-01") & date <= ymd("2021-06-30")) %>%
  # Simplify and restructure
  select(location, date, pseudo_nitzschia_delicatissima_group_cells_l, pseudo_nitzschia_seriata_group_cells_l) %>%
  gather(key="pn_group", value="cells_l", 3:4) %>%
  ...
  group_by(location, year, week, date_std, pn_group) %>%
  summarize(n=n(),
            cells_l_max=max(cells_l)) %>%
  ungroup() %>%
  # Arrange
...
# Export data
saveRDS(data_plot, file=file.path(outdir, "2014_2021_pn_density_by_pier_for_plotting.Rds"))
...
# Plot data
ymax <- max(data_plot$cells_l_max, na.rm=T) + 1
g <- ggplot() +
  # Seasons
  geom_rect(data=season_key, mapping=aes(xmin=open_date, xmax=close_date), ymin=-10, ymax=ymax, fill="grey90") +
  # Lines
  geom_line(data=data_plot, mapping=aes(x=date_std, y=cells_l_max+1, color=pn_group), lwd=0.2) +
  # Bloom threshhold reference lines
  geom_hline(yintercept=c(10^4), linetype="dotted", lwd=0.4) +
  # facet_wrap(pn_group~location, ncol=1) +
  facet_grid(location~pn_group) +
  # Labels
  labs(x="", y="Density (cells/L)") +
  # Scales
  scale_x_date(date_breaks = "1 year", date_labels="%Y") +
  scale_y_continuous(trans="log10",
                     breaks=10^c(0:5),
                     labels=paste0("10^", 0:5)) +
  # Legend

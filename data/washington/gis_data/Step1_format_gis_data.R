...
g <- ggplot() +
  ...
  geom_sf(data=usa, fill="grey80", color="white", lwd=0.3) +
  # Plot harvest sites
  geom_sf(data=growing_areas, mapping=aes(color=growing_area), show.legend = F) +
  # Crop
  coord_sf(xlim=c(-125.5,-121.5), ylim=c(46, 49.2)) +
  # Labels
  labs(x="", y="") +
...
# Export
saveRDS(harvest_sites, file=file.path(outdir, "WDFW_growing_areas.Rds"))
...
# Read key
zone_key <- readxl::read_excel(file.path(outdir, "WDFW_close_zone_key.xlsx")) %>%
...
if(F){
  closure_zones_df <- closure_zones %>%
    sf::st_drop_geometry()
  write.csv(closure_zones_df, file=file.path(outdir, "WDFW_close_zone_key_raw.csv"), row.names=F)
}

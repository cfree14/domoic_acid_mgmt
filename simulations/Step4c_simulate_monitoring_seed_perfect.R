...
results <- purrr::map_df(1:nrow(scenario_key), function(x){
  ...
  interval_wk_do <- scenario_key$interval_wk[x]
  ...
  output <- simulate_mgmt(toxin_grid_df,
                          ...
                          repeat_interval=interval_wk_do,
                          perfect=T, plot=F)
  ...
  results_i <- perf_stats %>%
    mutate(nstations=nstations_do,
           interval_wk=interval_wk_do,
           toxin_scenario=toxin_scenario,
           toxin_scenario_iter=toxin_scenario_iter) %>%
    select(nstations, interval_wk, toxin_scenario, toxin_scenario_iter, everything())
...
# Export
saveRDS(results, file=file.path(outputdir, "simulation_output_seeded_perfect.Rds"))
...
g <- ggplot(stats, aes(x=p_risk_missed_avg, y=p_close_unneeded_avg, fill=nstations)) +
  facet_grid(interval_wk~toxin_scenario, scales="free") +
  # Ranges
